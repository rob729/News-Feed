...
class NewsRepository(
    ...
    fun fetchNewsArticles(newsSourceDomain: String): Flow<NewsResource> {
        currentPage = 1
        return getNewsArticles(newsSourceDomain)
    }

    fun fetchMoreNewsArticles(newsSourceDomain: String): Flow<NewsResource> {
        ++currentPage
        return getNewsArticles(newsSourceDomain)
    }

    private fun getNewsArticles(newsSourceDomain: String): Flow<NewsResource> =
        flow {
            emit(NewsResource.Loading)
            val newsDbEntity =
                newsDBDataSource.getNewsFromNewsSourceDomain(newsSourceDomain, currentPage)
            val newsSourceFetchTimeInMillis =
                newsDBDataSource.getNewsSourceFetchTimeInMillis(newsSourceDomain)
            if (newsDbEntity == null || newsDbEntity.articles.isEmpty() ||
                checkIfNewsSourceDataIsOutdated(
    ...
    private suspend fun fetchNewsFromNetworkAndSyncDatabase(newsSourceDomain: String): NewsResource {
        try {
            ...
            return if (newsResource is NewsResource.Success<*> && newsResource.data is NewsApiResponse) {
                ...
                NewsResource.Success(
                    newsDBDataSource.getNewsFromNewsSourceDomain(
                        newsSourceDomain,
                        currentPage,
                    ),
                )
    ...
    fun getSearchResults(query: String): Flow<NewsResource> =
        flow {
            emit(NewsResource.Loading)
            try {
